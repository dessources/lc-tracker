-- Comeback Challenge fixes:
-- 1. `reviews.is_comeback` — flags a review as a Comeback Challenge resolution.
--    Lets every device agree on "a comeback was done today" (fixes the
--    challenge lingering on other devices after being done on one), and lets
--    the leaderboard stop double-counting a comeback as a normal solve.
-- 2. Points: a comeback now earns ONLY its comeback_bonus, not the normal
--    per-difficulty review points on top. The base point sums below exclude
--    is_comeback reviews.
--
-- Backfill: past aced comebacks are the ones with comeback_bonus > 0, so mark
-- them is_comeback too, so the double-count is removed retroactively.

alter table public.reviews add column if not exists is_comeback boolean not null default false;

update public.reviews set is_comeback = true where comeback_bonus > 0 and is_comeback = false;

-- ============================================================
-- Leaderboard: exclude comebacks from base difficulty points; keep bonus.
-- ============================================================
    ...
    (
      (select coalesce(sum(x.pts), 0)::bigint + 5 * count(distinct x.date)
       from (
         ...
         where r.user_id = p.id and r.date >= week_start and not r.is_comeback
       ) x)
      + (select coalesce(sum(r2.comeback_bonus), 0)::bigint
         from public.reviews r2 where r2.user_id = p.id and r2.date >= week_start)
    ),
    (
      (select coalesce(sum(x.pts), 0)::bigint + 5 * count(distinct x.date)
       from (
         ...
         from public.reviews r
         join public.problems pr on pr.id = r.problem_id
         where r.user_id = p.id and r.date >= week_start - 7 and r.date < week_start and not r.is_comeback
       ) x)
      + (select coalesce(sum(r2.comeback_bonus), 0)::bigint
         from public.reviews r2 where r2.user_id = p.id and r2.date >= week_start - 7 and r2.date < week_start)
    ),
    (
      (select coalesce(sum(x.pts), 0)::bigint + 5 * count(distinct x.date)
       from (
         ...
         from public.reviews r
         join public.problems pr on pr.id = r.problem_id
         where r.user_id = p.id and not r.is_comeback
       ) x)
      + (select coalesce(sum(r2.comeback_bonus), 0)::bigint
         from public.reviews r2 where r2.user_id = p.id)
    ),
    coalesce(
...
-- ============================================================
-- Nudge points: same exclusion so digest ranking matches the leaderboard.
-- ============================================================
      ...
      (
        (select coalesce(sum(x.pts), 0)::int + 5 * count(distinct x.date)
         from (
           ...
           from public.reviews r
           join public.problems pr on pr.id = r.problem_id
           where r.user_id = gm.user_id and r.date >= week_start and not r.is_comeback
         ) x)
        + (select coalesce(sum(r2.comeback_bonus), 0)::int
           from public.reviews r2 where r2.user_id = gm.user_id and r2.date >= week_start)
      ) as pts
    from public.group_members gm
