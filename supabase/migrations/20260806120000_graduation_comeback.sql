-- Pattern-of-work change: bound the review queue.
--
-- 1. `problems.graduated` — a problem that reaches a ~30-day interval has proven
--    mastery and leaves the active review queue (client-side scheduling; the
--    server just stores the flag). Backfilled from the old 30-day cap so
--    existing well-known problems graduate immediately for instant relief.
-- 2. `reviews.comeback_bonus` — bonus points awarded when a graduated problem is
--    aced in the daily Comeback Challenge. Additive: existing rows are 0, so no
--    historical points change. The leaderboard/nudge point sums add it on.
...
-- One-time backlog relief: anything already at the old 30-day ceiling has been
-- recalled enough to graduate.
update public.problems set graduated = true where interval_days >= 30 and graduated = false;

-- ============================================================
-- Leaderboard: fold comeback bonus into weekly / prev-week / all-time points.
-- (Redefines the America/New_York version from 20260612120000_local_time_windows.)
-- ============================================================
...
-- ============================================================
-- Nudge points: same weekly comeback bonus so digest ranking stays consistent.
-- ============================================================
